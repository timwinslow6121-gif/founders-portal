...
- ✅ Built local, NOT yet deployed: **S2 — Audit log + breach alerting** (branch `feat/s2-audit-alerting`, 198 tests pass). `app/audit.py` `log_event()` seam (logs login/data-view/export/admin/429 with IP/UA/agency/category/severity/record_count; defensive — never raises into caller), `app/alerts.py` 2 de-duped email triggers (non-domain/failed login + 429 flood; **exports are LOG-ONLY** — Tim works all hours so no off-hours rule), `/admin/audit-log` viewer, migration 025, `docs/INCIDENT_RESPONSE_RUNBOOK.md`. **NEXT ACTION: backup DB → merge to main → deploy to VPS (`flask db upgrade` runs 025) → verify (trigger a non-domain login → alert email arrives; check viewer shows rows).**
- 🔜 AFTER S2 deploy: **S3 — Encryption-at-rest** (Milestone 1 / Security pillar 4; see [[s3-encryption-preview]] — FIRST task = verify VPS disk encryption; then searchable-MBI column encryption). Then S4 pentest. Then Milestone 2 (UI/IA), Milestone 3 (#6 names).
...
- 📌 Repo: `main` synced with origin; S2 on branch `feat/s2-audit-alerting` (not yet merged). Migration head 025 (S2; runs on S2 deploy). Email = Brevo (raw `xkeysib-` key). DNS = nixihost.com (NOT cPanel).
