...
- ✅ Built local, NOT yet deployed: **S1 — Access hardening + session security** (branch `feat/s1-access-hardening`, 188 tests pass). `app/security.py`: 12h session timeout (matches Google Workspace 12h reauth), hardened/capped cookies, security headers (HSTS/CSP/X-Frame/etc.), Flask-Limiter w/ per-agent two-tier key. No migration, new dep `flask-limiter`. **NEXT ACTION: merge to main → deploy to VPS → verify (login works, theme loads light+dark, headers present, no mid-day logout).**
- 🔜 AFTER S1 deploy: **S2 — Audit log + breach alerting** (Milestone 1 / Security pillar 3; consumes S1's lockout/rate-limit events). Then S3 encryption-at-rest (see [[s3-encryption-preview]] — verify VPS disk encryption first), S4 pentest. Then Milestone 2 (UI/IA), Milestone 3 (#6 names).
