...
- ✅ Shipped & deployed (2026-06-10): **S1 — Access hardening + session security** (on main, VPS deployed + live-verified). `app/security.py`: 12h session timeout (matches Google Workspace 12h reauth), hardened/capped cookies, security headers (HSTS/CSP/X-Frame/etc.), Flask-Limiter w/ per-agent two-tier key. No migration, new dep `flask-limiter`. VERIFIED on live HTTPS: all 5 headers present (incl. HSTS = full ProxyFix→nginx TLS chain works), rate limiter trips to 429 (≈20/min effective = 10×2 workers, as-documented), nginx forwards X-Forwarded-For/-Proto. Only remaining = a human browser glance (Google login round-trip + light/dark render — header/200 checks imply OK).
- 🔜 NEXT: **S2 — Audit log + breach alerting** (Milestone 1 / Security pillar 3; consumes S1's lockout/rate-limit events). Then S3 encryption-at-rest (see [[s3-encryption-preview]] — verify VPS disk encryption first), S4 pentest. Then Milestone 2 (UI/IA), Milestone 3 (#6 names).
