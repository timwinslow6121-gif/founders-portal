...
- 📌 Repo: `main` synced with origin. Migration head **026** (widen commission_line_items.payment_type 32→256 for UHC quarantine descriptions; applied on VPS 2026-06-12). Email = Brevo (raw `xkeysib-` key). DNS = nixihost.com (NOT cPanel). VPS DB backup: `PGPASSWORD=<from .env DATABASE_URL> pg_dump -U founders_user -h localhost founders_portal` (NOT plain `pg_dump` — that uses role 'root' which doesn't exist).
