...
RESOLUTION ORDER (see docs/superpowers/specs/2026-08-06-ledger-customer-backlink-repair-design.md):
  1. (statement_id, source_ref) -> PolicyPayment -> Policy.customer_id. The
     payment sibling already resolved at ingest with the full tier stack + the
     carrier crosswalk, so the ledger INHERITS its answer and the two tables
     cannot diverge. Measured: recovers 537 of 641, a strict superset of
     re-resolving.
...
WHY THE KEY IS COMPOSITE: `source_ref` schemes (see ledger.py's per-carrier
extractors) carry a carrier tag, a per-agent/file token, a sheet name, and a
row index — but no period or statement id. `aetna::0::147` is the identical
string in every monthly Aetna file; it just means "row 147 of the Aetna
sheet". source_ref is only unique WITHIN a statement, never across them. This
is not a quirk of this module — it is the schema's own contract: the
`uq_payment_statement_source_ref` / `uq_lineitem_statement_source_ref`
constraints (models.py) are composite on (statement_id, source_ref), and every
other consumer (ingest.py, ledger.py's own re-edit lookups) already scopes by
statement_id. A single-key map here silently let a later statement's row
inherit an earlier statement's customer_id whenever the two shared a
source_ref string — confirmed on production: 3,993 source_ref values span
more than one statement, 2,363 of which map to more than one distinct
customer.
"""
...
def build_backlink_context(agency_id):
    ...
    # Step-1 map: (statement_id, source_ref) -> customer_id, via the
    # already-resolved payment row. Composite key: source_ref alone is only
    # unique within a statement (see module docstring).
    by_source_ref = {}
    rows = (db.session.query(PolicyPayment.statement_id, PolicyPayment.source_ref,
                             Policy.customer_id)
            ...
            .all())
    for stmt_id, sref, cust_id in rows:
        if sref:
            by_source_ref[(stmt_id, sref)] = cust_id
...
def resolve_customer_id(ctx, *, statement_id, source_ref, carrier, mbi,
    ...
    `statement_id` is REQUIRED — source_ref alone is not a unique key across
    statements (see module docstring); omitting it would let one statement's
    row resolve against a different statement's row that happens to share the
    same source_ref string.
    """
    # 1. The payment sibling already did the work.
    sref = (source_ref or "").strip()
    if sref:
        hit = ctx.by_source_ref.get((statement_id, sref))
        if hit is not None:
            return hit
