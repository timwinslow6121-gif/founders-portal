...
- **Envelope/postage presentation is a HARD vendor criterion (not just price).** The mail must **feel personal, NOT transactional** — Founders' letters explicitly warn seniors about scammy solicitations, so a double-window "looks-like-a-bill" envelope undercuts the message and the personal touch Brian wants. **LetterStream is therefore a weaker fit despite its price/volume** — its standard is a double-window #10 with metered postage. Preferred: **closed-face envelope + a real (live) first-class stamp**, name printed (or handwriting-style) on the envelope (Click2Mail, The Addressers, PCM-with-options). A **handwritten** tier (Scribeless/SimplyNoted, ~$1.50–3+/pc) is likely overkill for full volume but could be reserved for high-value segments. Direct-mail research: live stamp + closed-face + no teaser copy = the biggest warm/senior open-rate lifts.
- **Procurement note (business, not code — Tim + AJ own this):** ⏳ **AJ is pulling last year's actual AEP numbers (piece count + total cost)** to serve as the baseline. Then: get real per-piece quotes from bulk vendors that offer **closed-face + live-stamp** for Founders' seasonal volume (~5k own book + pharmacy blasts, tens of thousands total); compare on **total-season cost AND presentation**, not price alone. This does NOT gate the build — the interface is vendor-neutral and the adapter is a thin, late phase; the vendor can be chosen right before that phase.
...
6. **Vendor + envelope presentation (Tim + AJ).** ⏳ AJ pulling last year's piece count + cost as the baseline. Then choose a **high-volume, no-cap, pay-per-piece** bulk mailer that offers **closed-face + live-stamp** (personal, not double-window/transactional). Candidates: Click2Mail, The Addressers, PCM; LetterStream only if double-window is accepted. Decided at quote time; adapter is late-phase. (See §6 for the hard criterion.)
