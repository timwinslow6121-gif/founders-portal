...
### The schedule records what was ACTUALLY PAID

Rate and tier can move independently from year to year, so a model assuming
"year N implies rate X and tier Y" cannot represent what actually happened.
`AgentContractRate` rows therefore record the terms **as applied** for each
period, not a formula.

The historical schedule is reconstructed from what was paid, not assumed:
`CommissionLineItem.split_rate` is frozen per row, so the rate in force in any
past period can be read straight off the ledger.
