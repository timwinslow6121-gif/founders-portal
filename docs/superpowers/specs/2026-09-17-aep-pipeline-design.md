...
| **Carrier enrollment confirmation feeds** | no API for the carriers themselves; but see IntegrityCONNECT below — Medicare Center is no longer email-only |

## IntegrityCONNECT Leads Partner API (added 2026-09-18)

Integrity opened a REST API over Medicare Center: *"leads, addresses, contact
information, and health profiles … NPN-scoped … machine-to-machine."* This removes
the weakest dependency in the design — the plan to have every agent set up a Gmail
forwarding rule for carrier confirmation emails and never break it.

### What the docs establish

- **Auth:** client-credentials M2M. `POST /ae-partner-auth-service/auth/token` with
  `client_id` + `client_secret` → Bearer token, **`expires_in: 900` (15 min), no
  refresh token.** Re-request before expiry.
- **Base:** `https://ae-api.integrity.com/api/v3.0/partners/…`; first endpoint
  `GET /partners/leads`. **Sandbox is live and separate from production.**
- **Scopes:** `PartnerRead` / `PartnerWrite` / `PartnerFull`. **We want `PartnerRead`
  only** — the portal should never write back into Medicare Center. Write access
  invites a two-master problem over customer data we already treat as authoritative.
- **Lead shape (from the docs' example):** `leadId`, `firstName`, `lastName`, `email`,
  `phone`, `stage` (an integer), `createdDate`.

### ⚠ The binding constraint: one credential = one NPN

Stated twice and *"enforced at the gateway and cannot be overridden"* — the credential
binds to the NPN **extracted from the login session of whoever generates it**. It is
not an agency-wide key by default.

Founders' book is split across nine producers:

| Agent | Active policies |
|---|---|
| Brian Freeman | 1,365 |
| Rebekah Long | 1,155 |
| Chris Foster | 770 |
...
**Founders' agency NPN is `20388847`** (Tim, 2026-09-18), now stored on `Agency.npn`
(migration 045). Each producer's NPN is on `User.npn` (migration 044).

🔴 **The unresolved question, and the first thing to test in sandbox: does a credential
generated under the agency NPN return downline producers' leads, or only leads written
under the agency NPN itself?** The docs never mention hierarchy, upline/downline, or
agency scope — only *"data accessible to your NPN"*. Two possibilities:

- **Hierarchy is honoured** → one agency credential covers the book. Simple.
- **Hierarchy is not honoured** → **nine credentials**, each generated by that producer
  logging into the Developer Portal themselves, each with its own secret to store and
  rotate. Brian and Rebekah alone hold 46% of the book, and neither is technical.

**Do not design the integration until this is answered.** The two shapes differ by an
order of magnitude in both build cost and operational burden.

### Open questions for the API itself

- **Leads only, or enrollments too?** The pipeline needs *enrollment confirmations* —
  `channel="enrollment"`, outcome `enrolled`. The quickstart shows only `/partners/leads`.
  If applications are not exposed, this improves lead capture but does **not** close the
  enrollment-feed gap, and the spec's `enrolled` outcome stays manual.
- **What is `stage`?** An integer with no documented enum. It may or may not map onto the
  pipeline's own stages. Do not guess a mapping.
- **Pull or push?** No webhooks documented. Pull means a poll interval and a
  `since`/cursor parameter — the quickstart's `GET /partners/leads` shows neither, so
  pagination and incremental sync are unknown.
- **"Health profiles"** is in the announcement but absent from the quickstart. If it
  carries LIS/Medicaid level it addresses `medicaid_level` being set on **1 of 5,495
  customers** — the reason D-SNP segmentation is out of scope — which matters now that
  896 policies are moving to the renamed D-SNPs (`H5253-216` / `H5253-217`).
- **Rate limits** — not documented. Nine credentials × a poll interval needs a budget.

### Secret handling (non-negotiable)

Nine client secrets is nine secrets. They go in the VPS `.env`, never in git, and the
15-minute token is cached in memory, never persisted. This is the same rule that applies
to the Quo keys — and note those **still need rotating**, having passed through chat.

### Next step: a sandbox spike, not a build

1. Generate **sandbox** credentials under the agency NPN `20388847`.
2. `GET /partners/leads` and count what comes back.
3. Compare against a known single-producer book (Tim's 553) to tell hierarchy-honoured
   from agency-only.
4. Inspect one full lead payload for enrollment status, health profile, and `stage`.

That is a half-day spike whose output is an answer, not code.
