...
SEED = [
    ...
    # --- In-progress ---------------------------------------------------------
    dict(type="feature", status="in_progress", priority="high",
         title="Smarter customer & plan records (one person, all their plans)",
         issue_text="The portal needs to always keep one record per real person, hold their correct current Medicare ID, and list every plan they have — including ones no carrier feed can see.",
         fix_text="We're building the rules for how a customer's plans fit together (which coexist, which replace each other) so records stay clean and complete."),

    # --- Planned (the coming wins) ------------------------------------------
    dict(type="planned", status="planned", priority="high",
         title="AEP mailer — generate and send AEP letters to your book",
         issue_text=None,
         fix_text="Pick your plans, generate a letter, and have it printed and mailed to your customers for you."),
    dict(type="planned", status="planned", priority="high",
         title="Redesigned plan-summary page + fast drug-tier checker",
         issue_text=None,
         fix_text="A cleaner plan page with a Consumer / Pro view, in-network vs out-of-network toggle, and a quick way to look up which tier a drug falls in."),
    dict(type="planned", status="planned", priority="medium",
         title="Filter plans by Part-B giveback",
         issue_text=None,
         fix_text="A filter to find the plans that give money back on the Part-B premium — increasingly what customers ask for."),
    dict(type="planned", status="planned", priority="medium",
         title="Flag customers with retirement / secondary coverage",
         issue_text=None,
         fix_text="Mark and filter customers who also have SHP (retired teachers/state), military (TRICARE/VA/CHAMPVA), or federal (FEHB) coverage — it explains their plan choice."),
    dict(type="planned", status="planned", priority="high",
         title="Log the free PDP signups you do",
         issue_text="When you set up a customer's drug plan for free (e.g. via Medicare.gov), there's nothing in the portal tying that work to Founders.",
         fix_text="A way to record 'I enrolled this person in PDP X on this date' so the service is captured — good for follow-up and proof of work."),
    dict(type="planned", status="planned", priority="medium",
         title="Cleaner sidebar / navigation",
         issue_text="The left navigation has grown cluttered and some labels are confusing.",
         fix_text="A reworked sidebar — grouped, reordered, and renamed around how you actually think about the work."),
    dict(type="planned", status="planned", priority="medium",
         title="Customer plan-history timeline",
         issue_text=None,
         fix_text="A profile view showing the customer's current plan plus their past coverage as a timeline (plan, type, dates)."),
    dict(type="planned", status="planned", priority="low",
         title="'Back to where I came from' navigation",
         issue_text="Drilling from commissions into a customer profile leaves no easy way back to the page you came from.",
         fix_text="Breadcrumbs / a contextual back-link so you can return to exactly where you were."),

    # --- Known issues / current limitations ---------------------------------
    dict(type="known_issue", status="acknowledged", priority="medium",
         title="Some carrier books are still being reconciled",
         issue_text="Devoted, BCBS, Wellabe and GTL haven't been fully reconciled against their official books yet, so their active counts may not be exact.",
         fix_text=None),
    dict(type="known_issue", status="acknowledged", priority="low",
         title="A few UHC commission rows still need manual review",
         issue_text="A small number of UHC 'New enrollment' commission rows are set aside for manual review rather than auto-split, pending confirmation of the exact math.",
         fix_text=None),
]
